...
X_Validate=ValX_unshapedDsmall.reshape(Number_of_Batches_Val,Timesteps,ValX_unshapedDsmall.shape[1])  #,
...
Y_Validate  = ValY_unshapedDsmall
...
neurons=[100]
dropout=[0.1]
regularization=[.0001] #CHOOSE REGURALIZER
batch= [403]
loss= np.empty([NEpochs, repetitions])
valLoss= np.empty([NEpochs, repetitions])
val_AUC= np.empty([0, X_train.shape[2]])
# BUILD MODEL & TRAIN
for yy in range(0,len(batch)):
    for uu in range(0,repetitions):
        print("Making LSTM model")
        
        model = Sequential()
        #model.add(Bidirectional(LSTM(neurons[0],input_shape=(X_train.shape[1],X_train.shape[2]))))
        model.add(LSTM(neurons[0], activation='sigmoid',input_shape=(X_train.shape[1],X_train.shape[2]))) #return_sequences=True,
        ...
        # model.add(Dense(256))
        # model.add(Dropout(0.3))
        model.add(Dense(X_train.shape[2],kernel_regularizer=regularizers.l2(regularization[0])))
        ...
        history=model.fit(X_train, Y_train, validation_data=(X_Validate, Y_Validate),  batch_size=batch[yy], epochs=NEpochs, verbose=1,callbacks=[roc_callback(validation_data=(X_Validate, Y_Validate))])
    ...
    np.savetxt('C:/Users/Apoorva.Radhakrishna/Documents/Rocmatt/EC2BACKUP/SavedModels/ValAUCbatchBiDel'+ str(yy) + '.txt', val_AUC)
    np.savetxt('C:/Users/Apoorva.Radhakrishna/Documents/Rocmatt/EC2BACKUP/SavedModels/LossbatchBiDel'+ str(yy) + '.txt', loss)
    np.savetxt('C:/Users/Apoorva.Radhakrishna/Documents/Rocmatt/EC2BACKUP/SavedModels/ValLossbatchBiDel'+ str(yy) + '.txt', valLoss)
...
#***************************************************************************************************************************************************************
   ...
   TestInput_Dense=TestInput_unshaped.toarray()
   TestInput_Densesmall=TestInput_Dense[:,Col2Select]
   print(TestInput_Dense.shape)
   print(TestInput_Densesmall.shape)
   TestInput=TestInput_Densesmall.reshape(Number_of_Batches_Test,Timesteps,TestInput_Densesmall.shape[1])
