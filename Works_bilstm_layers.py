...
Train      = scipy.sparse.load_npz("D:/Inputs/mixie/Train.npz")
Test       = scipy.sparse.load_npz("D:/Inputs/mixie/Test.npz")
Val        = scipy.sparse.load_npz("D:/Inputs/mixie/Val.npz")
...
Input_unshaped    = scipy.sparse.load_npz("D:/Inputs/mixie/TrainX_10_s5.npz")
...
print(X_train.shape)
Target_unshaped   = scipy.sparse.load_npz("D:/Inputs/mixie/TrainY_10_s5.npz")
Y_train           = Target_unshaped.toarray()
print(Y_train.shape)

ValX_unshaped   = scipy.sparse.load_npz("D:/Inputs/mixie/ValX_10_s5.npz")
...
ValY_unshaped   = scipy.sparse.load_npz("D:/Inputs/mixie/ValY_10_s5.npz")
...
TestInput_unshaped = scipy.sparse.load_npz("D:/Inputs/mixie/TestX_10_s5.npz")
...
TestOuput_unshaped = scipy.sparse.load_npz("D:/Inputs/mixie/TestY_10_s5.npz")
TestOuput_Dense    = TestOuput_unshaped.toarray()
np.savetxt("D:/Val/Target.csv", TestOuput_Dense, delimiter=",") 
...
model = Sequential()
#model.add(Bidirectional(LSTM(700, activation='relu', dropout=0.9536,return_sequences=True),merge_mode='sum',input_shape=(X_train.shape[1],X_train.shape[2])))
#model.add(Bidirectional(LSTM(500, activation='relu', dropout=0.1374,return_sequences=True),merge_mode='sum',input_shape=(X_train.shape[1],X_train.shape[2])))
#model.add(Bidirectional(LSTM(700, activation='relu', dropout=0.587,return_sequences=True),merge_mode='sum',input_shape=(X_train.shape[1],X_train.shape[2])))
model.add(Bidirectional(LSTM(300, activation='relu', dropout=0.4844),merge_mode='sum',input_shape=(X_train.shape[1],X_train.shape[2])))
model.add(Dense(X_train.shape[2], activation='sigmoid',kernel_regularizer=regularizers.l2(0.001)))
...
history=model.fit(X_train, Y_train, validation_data=(X_Validate, Y_Validate),  batch_size=803, epochs=NEpochs, verbose=1,callbacks=[roc_callback(validation_data=(X_Validate, Y_Validate)),es])
...
for gg in range(0,len(Val_AUC)):
    temp=np.asarray(Val_AUC[gg])
    val_AUC=np.vstack((val_AUC,np.transpose(temp)))
        

np.savetxt('D:/Val/b1367_AUC.csv', val_AUC)
np.savetxt('D:/Val/b1367_Loss.txt', Loss)
np.savetxt('D:/Val/b1367_ValLoss.txt', ValLoss)
...
AUC_Score=sklearn.metrics.roc_auc_score(TestOuput_Dense.flatten(), PredictedRawProbabilities.flatten())
np.savetxt("C:/Users/Apoorva.Radhakrishna/Desktop/Shareernalam/validation/b1367_PredictedRawProbabilities.csv", PredictedRawProbabilities, delimiter=",") 
print(AUC_Score)
