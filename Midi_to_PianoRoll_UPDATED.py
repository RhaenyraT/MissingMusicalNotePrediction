...
piano_roll = np.zeros((TotalTime, 87)) #SUPPOSED TO BE 88 :O
for row in Note_StartTime_Length:
    piano_roll[row[1]:(row[1]+row[2]), row[0]] = 1

Pianoroll=sparse.csr_matrix(piano_roll)
scipy.sparse.save_npz("C:/Users/Apoorva.Radhakrishna/Documents/Rocmatt/EC2BACKUP/EC2/mz5703.npz", Pianoroll, compressed=True)
