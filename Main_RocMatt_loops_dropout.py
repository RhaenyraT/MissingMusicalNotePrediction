...
for yy in range(0,len(dropout)):
    for uu in range(0,repetitions):
        ...
        history=model.fit(X_train, Y_train, validation_data=(X_Validate, Y_Validate),  batch_size=413, epochs=NEpochs, verbose=1,callbacks=[roc_callback(validation_data=(X_Validate, Y_Validate))])
    ...
    #np.savetxt('C:/Users/Apoorva.Radhakrishna/Documents/Rocmatt/EC2BACKUP/SavedModels/ValAUCDropout'+ str(neurons[yy]) + '.txt', val_AUC)
    #np.savetxt('C:/Users/Apoorva.Radhakrishna/Documents/Rocmatt/EC2BACKUP/SavedModels/LossDropout'+ str(neurons[yy]) + '.txt', loss)
    #np.savetxt('C:/Users/Apoorva.Radhakrishna/Documents/Rocmatt/EC2BACKUP/SavedModels/ValLossDropout'+ str(neurons[yy]) + '.txt', valLoss)
