...
TestOuput_unshaped = scipy.sparse.load_npz("C:/Users/Apoorva.Radhakrishna/Documents/Rocmatt/EC2BACKUP/EC2/Inputs/mixie/TestY_10_s5.npz")
TestOuput_Dense    = TestOuput_unshaped.toarray()
np.savetxt("C:/Users/Apoorva.Radhakrishna/Desktop/Shareernalam/validation/20Target.csv", TestOuput_Dense, delimiter=",") 
...
for uu in range(0,repetitions):
    ...
    model = Sequential()
    #model.add(Bidirectional(LSTM(700, activation='tanh', dropout=0.9536,return_sequences=True),merge_mode='sum',input_shape=(X_train.shape[1],X_train.shape[2])))
    #model.add(Bidirectional(LSTM(500, activation='tanh', dropout=0.1374,return_sequences=True),merge_mode='sum'))
    #model.add(Bidirectional(LSTM(700, activation='tanh', dropout=0.587,return_sequences=True),merge_mode='sum'))
    model.add(Bidirectional(LSTM(300, activation='relu', dropout=0.4844),merge_mode='sum'))
    ...
    history=model.fit(X_train, Y_train, validation_data=(X_Validate, Y_Validate),  batch_size=118929, epochs=NEpochs, verbose=1,callbacks=[roc_callback(validation_data=(X_Validate, Y_Validate)),es])
    #history=model.fit(X_train, Y_train, validation_data=(X_Validate, Y_Validate),  batch_size=413, epochs=NEpochs, verbose=1)
    #model_json = model.to_json()
   # with open("C:/Users/Apoorva.Radhakrishna/Desktop/Shareernalam/validation/biSum_model.json", "w") as json_file:
    #    json_file.write(model_json)
    # serialize weights to HDF5
    #model.save_weights("C:/Users/Apoorva.Radhakrishna/Desktop/Shareernalam/validation/biSum_model.h5")
    ...
    np.savetxt('C:/Users/Apoorva.Radhakrishna/Desktop/Shareernalam/validation/118929_'+str(uu)+'_AUC.csv', val_AUC)
    np.savetxt('C:/Users/Apoorva.Radhakrishna/Desktop/Shareernalam/validation/118929_'+str(uu)+'_Loss.txt', Loss)
    np.savetxt('C:/Users/Apoorva.Radhakrishna/Desktop/Shareernalam/validation/118929_'+str(uu)+'_ValLoss.txt', ValLoss)
    ...
    np.savetxt('C:/Users/Apoorva.Radhakrishna/Desktop/Shareernalam/validation/118929_'+str(uu)+'_PredictedRawProbabilities.csv', PredictedRawProbabilities, delimiter=",") 
    print(AUC_Score)
